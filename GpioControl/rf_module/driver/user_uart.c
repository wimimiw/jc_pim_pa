...
{		
	...
	{//receive interrupt
		...
		if(__Uart1_Callback != NULL)
		{
			__Uart1_Callback(uartBufQS[UART1].buf,uartBufQS[UART1].bufDataLen);
		}				
	}
	else
	{//exception 
...
/**
 ...
 {		
 	...
 	{//receive interrupt					
		...
		if(__Uart2_Callback != NULL)
		{
			__Uart2_Callback(uartBufQS[UART2].buf,uartBufQS[UART2].bufDataLen);
		}	
 	}
 	else
 	{//exception 
...
void UartTxOpen(U8 bus,U16 len)
{
	...
	{
		DRIVER_DIRECT_ENABLE();
		if(bus == UART1)USART_SendData(USART1,*pt);
